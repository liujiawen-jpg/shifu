...
path='C:\\Users\\admin\\Desktop\\Artifical\\new\\artificial\\data\\trainData'
a='/'

data_dir = 'C:\\Users\\admin\\Desktop\\Artifical\\new\\artificial\\data'
...
# 进入图片数据的目录，读取图片信息
all_data_files = os.listdir(os.path.join(data_dir, 'trainData/'))
...
for each in all_test_files:
    ...
    #img = cv2.imread(os.path.join(data_dir,'trainData/',each), 1)
    im= Image.open(path+a+each) 
    img = im.resize((50,50))
